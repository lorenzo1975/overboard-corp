      ...
      <main className="flex-1 container mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          ...
          <p className="mt-4 text-lg text-muted-foreground max-w-3xl mx-auto">
            From a single shop to a leading lifestyle brand, our journey has been one of passion, resilience, and a deep love for the water.
          </p>
        </div>
          ...
          {timelineEvents.map((event, index) => (
            <div key={event.year} className="relative mb-12 md:mb-24">
              <div className="md:flex items-center md:gap-12">
                ...
                <div className={`md:w-1/2 mt-8 md:mt-0 ${index % 2 === 0 ? 'md:order-2' : 'md:order-1'}`}>
                  <div className={`relative ${index % 2 === 0 ? 'md:pl-12' : 'md:pr-12 md:text-right'}`}>
                     <div className="absolute top-1/2 -translate-y-1/2 h-4 w-4 rounded-full bg-accent ring-8 ring-background hidden md:block"
                       style={index % 2 === 0 ? { left: 0, transform: 'translate(-50%, -50%)' } : { right: 0, transform: 'translate(50%, -50%)' }}
                     />
                    <p className="text-sm font-semibold uppercase tracking-wider text-accent mb-1">{event.year}</p>
                    <h2 className="text-3xl font-bold font-headline tracking-tight mb-4">{event.title}</h2>
                    <p className="text-muted-foreground">{event.description}</p>
                  </div>
                </div>
              </div>
              {index < timelineEvents.length - 1 && <Separator className="md:hidden mt-12"/>}
            </div>
          ))}
        </div>
      </main>
